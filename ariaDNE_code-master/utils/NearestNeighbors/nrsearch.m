...
%    NNInfo.Atria or NNInfo.CoverTree will be populated if TSTool or covertrees (resp.) where used.
%
%
...
lUseCoverTrees = false;                     %%%%% FOR DEBUG ONLY
...
if lUseCoverTrees,
    if ~isfield(opts,'CoverTree'),
        % Construct cover tree if needed
        T_opts=struct('theta',0.5,'numlevels',int32(100),'minlevel',int32(0),'NTHREADS',int32(feature('numCores')),'BLOCKSIZE',int32(1024));
        NNInfo.CoverTree=covertree(T_opts,cX);
    else
        NNInfo.CoverTree = opts.CoverTree;
    end;
end;
