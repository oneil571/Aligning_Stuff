%% Global analysis using MDS Diagrams

%% Load MDS information and extract distances to means
...
touch([interStatPath 'Total/MDS/Quad/'])
quadid = fopen([interStatPath 'Total/MDS/Quad/LeveneTests.csv'],'w');
fprintf(quadid,'Specimen1\tSpecimen2\tP_value\n');

touch([interStatPath 'Total/MDS/Abs/'])
absid = fopen([interStatPath 'Total/MDS/Abs/LeveneTests.csv'],'w');
...
for i = 1:length(SpecimenTypes)-1
    for j = (i+1):length(SpecimenTypes)
        ...
        quadP = [quadP P];
        touch([interStatPath 'Total/MDS/Quad/']);
        save([interStatPath 'Total/MDS/Quad/'...
        ...
        absP = [absP P];
        touch([interStatPath 'Total/MDS/Abs/']);
        save([interStatPath' 'Total/MDS/Abs/'...
            Specimen1 '_' Specimen2 '.mat'],'P');
    end
end
...
for k = 1:length(keys)
    ...
    %Setup file paths
    writePath = [interStatPath keys{k} '/MDS/'];
    touch(writePath);
...
for i = 1:length(SpecimenTypes)
    ...
    quadDict = {}; absDict = {}; quadP = []; absP = [];
    writePath = [intraStatPath SpecimenTypes{i} '/MDS/'];
    ...
    for k = 1:length(keys)
        ...
        for p = 1:length(curUnLabels)
            ...
            curDists = zeros(length(inds),1);
            for m = 1:length(inds)
                curDists(m) = norm(curMDS(inds(m),:)-curMean);
            end
            dists{p} = curDists;
        end
