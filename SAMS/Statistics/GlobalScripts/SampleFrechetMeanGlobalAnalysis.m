...
for i = 1:length(SpecimenTypes)
    curMean = min(find(sum(distCollection{i}.^2)==min(sum(distCollection{i}.^2))));
    curDists = zeros(length(meshCollection{i}),1);
    for j = 1:length(meshCollection{i})
        curDists(j) = norm(meshCollection{i}{j}.V-meshCollection{i}{curMean}.V,'fro');
    end
    dists{i} = curDists;
end

%% Levene tests for each pair of MetaGroups: prints nothing if only one group
touch([interStatPath 'Total/FrechetMean/Quad/']);
quadid = fopen([interStatPath 'Total/FrechetMean/Quad/LeveneTests.csv'],'w');
fprintf(quadid,'Specimen1\tSpecimen2\tP_value\n');
touch([interStatPath 'Total/FrechetMean/Abs/']);
absid = fopen([interStatPath 'Total/FrechetMean/Abs/LeveneTests.csv'],'w');
...
for i = 1:length(SpecimenTypes)-1
    for j = (i+1):length(SpecimenTypes)
        ...
        quadP = [quadP P];
        touch([interStatPath 'Total/FrechetMean/Quad/']);
        save([interStatPath 'Total/FrechetMean/Quad/'...
        ...
        absP = [absP P];
        touch([interStatPath 'Total/FrechetMean/Abs/']);
        save([interStatPath' 'Total/FrechetMean/Abs/'...
            Specimen1 '_' Specimen2 '.mat'],'P');
    end
end
...
for k = 1:length(keys)
    ...
    for i = 1:length(SpecimenTypes)
        for j = 1:length(totalUnLabels)
            ...
            curMeshes = meshCollection{i}(inds);
            curMean = min(find(sum(curDistMatrix.^2)==min(sum(curDistMatrix.^2))));
            curDists = zeros(length(curMeshes),1);
            for m = 1:length(curMeshes)
                curDists(m) = norm(curMeshes{m}.V-curMeshes{curMean}.V,'fro');
            end
            dists{i,j} = curDists;
        end
    end
    %Setup file paths
    writePath = [interStatPath keys{k} '/FrechetMean/'];
    touch(writePath);
...
for i = 1:length(SpecimenTypes)
    ...
    writePath = [intraStatPath SpecimenTypes{i} '/FrechetMean/'];
