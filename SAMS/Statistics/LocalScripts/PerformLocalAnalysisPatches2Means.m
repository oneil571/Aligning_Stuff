...
for s = 1:length(SpecimenTypes)
    ...
    for g = 1:length(Groups.keys)
        ...
        for p = 1:length(pValues)
            ...
            touch([curPath 'HeatMaps/' keys{g} '/PatchKMeans/' alignmentMethod '/Radius_' num2str(radius) '/']);
            for c = 1:length(FPC)
                touch([curPath 'HeatMaps/' keys{g} '/PatchKMeans/' alignmentMethod '/Radius_' ...
                    num2str(radius) '/' FPC{c} '/']);
                touch([curPath 'HeatMaps/' keys{g} '/PatchKMeans/' alignmentMethod '/Radius_' ...
                    num2str(radius) '/' FPC{c} '/' num2str(pValues(p)*10^(exp)) 'e-' num2str(exp) '/']);
        ...
        for i = 1:length(labelGroups)
            for j = 3%(i+1):length(labelGroups)
                ...
                testPath = [localPath 'PatchKMeans/' alignmentMethod '/Radius_' num2str(radius) '/'];
                touch(testPath);
                startInd = 1;
                ...
                for v = startInd:length(embedCoords)
                    [~,~,pVal(v),HTrue] = BruteForce2Means(embedCoords{v}([inds1 inds2],:)',...
                        1:length(inds1),(length(inds1)+1):(length(inds1)+length(inds2)));
                    disp([num2str(v) ': P = ' num2str(pVal(v)) '; H = ' num2str(HTrue)]);
                    if rem(v,10) == 0
                        save([testPath unLabels{i} '_' unLabels{j} '.mat'],'pVal');
                        disp(v)
                    end
                    progressbar(v/length(embedCoords));
                end
%                end
                [sortP,idx] = sort(pVal);
                for p = 1:length(pValues)
                    disp([unLabels{i} ' vs. ' unLabels{j} ' ' num2str(pValues(p))]);
                    for c = 1:length(FPC)
                        curFPC = FPC{c};
                        switch curFPC
                            case 'FWER'
                        ...
                        figPath = [curPath 'HeatMaps/' keys{g} '/PatchKMeans/' alignmentMethod '/Radius_'...
