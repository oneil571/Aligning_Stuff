...
classdef OptimProblemIsoDist < OptimProblem
    ...
    methods
        function obj = OptimProblemIsoDist(V, F, eq_lhs, eq_rhs, V0, initArapIter)
            ...
            assert(all(obj.areas>0),'Non-positive areas?!?');
...
% % % %             % --------------------------------
% % % %             % HACK: try a global reflection
% % % %             % --------------------------------
% % % %             currNumFlips = obj.flips;
% % % %             reflectMat = eye(obj.dim); reflectMat(end)=-1;
% % % %             x0 = x0*reflectMat;
% % % %             obj.Tx = obj.T*x0(:);
% % % %             [~, ~, obj.flips] = computeFunctionalIsoDistMex(obj.Tx, obj.areas, obj.dim);
% % % %             % keep smaller number of flips
% % % %             if currNumFlips<obj.flips
% % % %                 x0 = x0*reflectMat;
% % % %                 obj.Tx = obj.T*x0(:);
% % % %                 [~, ~, obj.flips] = computeFunctionalIsoDistMex(obj.Tx, obj.areas, obj.dim);
% % % %             end
