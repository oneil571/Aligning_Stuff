function patchtexture(FF,VV,TF,VT,I,Options)
...
Options.CDataMapping='direct';
Options.FaceLighting = 'flat';
Options.EdgeColor='none';
Options.FaceAlpha=1;
Options.AmbientStrength=0.5;
Options.SpecularStrength = 1;
...
for i=1:size(FF,1)
    ...
    oldpos(:,1)=xy(1,1)*lambda1+xy(2,1)*lambda2+xy(3,1)*lambda3;
    oldpos(:,2)=xy(1,2)*lambda1+xy(2,2)*lambda2+xy(3,2)*lambda3;
    pos=round(oldpos); pos=max(pos,1); pos(:,1)=min(pos(:,1),size(I,1)); pos(:,2)=min(pos(:,2),size(I,2));
    posind=(pos(:,1)-1)+(pos(:,2)-1)*size(I,1)+1;
    
    % Map texture to surface image
    badInds = find(lambda1 < 0); badInds = [badInds find(lambda2 < 0)];
    badInds = [badInds find(lambda3 < 0)];
    Jr(jind)=Ir(posind);
    J(:,:,1)=Jr; 
    if(iscolor)
        Jg(jind)=Ig(posind); 
