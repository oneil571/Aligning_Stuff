function [Umin,Umax,Cmin,Cmax,Cmean,Cgauss,Normal] = ComputeCurvature(G,options)
...
%yaron fix: remove indices greater than m
A(A>m) = 0;
...
% normals to faces
[~,normal] = G.ComputeNormal(vertex,face);
...
for x=1:3
    for y=1:3
        a = Tv(x,y,:);
        a = G.PerformMeshSmoothing(a(:),options);
