function [rslt] = RecomputeCPMapWithGP(GM,GN,options)
...
%% Temporary code for visualization, all changed immediately reverted
% if ref12==1
%     TextureCoords1(2,:) = -TextureCoords1(2,:);
%     TextureCoords2(2,:) = -TextureCoords2(2,:);
% end
% 
% flatM = Mesh('VF',TextureCoords1,GM.F);
% figure
% flatM.draw;
% pause()
% 
...
if strcmpi(FeatureMatchType,'TPS')
    if isfield(options,'GPMatches')
        %[TextureCoords1,cPmap] = TPSDeformationStableRobust(GM,GN,cPmap,'ConfMax',[real(pushGM);imag(pushGM)],TextureCoords2,ref12,options);
        [TextureCoords1,cPmap] = TPSDeformationGP(GM,GN,cPmap,'GP',[real(pushGM);imag(pushGM)],TextureCoords2,ref12,options);
    end
