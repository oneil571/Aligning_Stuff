function [TextureCoords1,ImprMap] = TPSDeformationStableRobust(GM,GN,InputMap,FeatureType,TextureCoords1,TextureCoords2,ref12,options)
...
%% ROB RAVIER added portion: Only interpolate if chosen vertices aren't too far away
...
vertsToDelete = [];
maxDist = .3;
for i = 1:length(TPS_FEATURESM_INDS)
    testDistance = norm(tempTextureCoords1(:,TPS_FEATURESM_INDS(i))-tempTextureCoords2(:,TPS_FEATURESN_INDS(i)));
    if testDistance > maxDist
        vertsToDelete = [vertsToDelete i];
    end
end

TPS_FEATURESM_INDS(vertsToDelete) = [];
TPS_FEATURESN_INDS(vertsToDelete) = [];
