...
function PrintPseudolandmarksSampleMean(workingPath,numGPLmks,numFPSLmks)

load([workingPath 'newMeshList.mat']);
load([workingPath 'FinalDists.mat']);

%% Metric Frechet mean
%[~,frechMean] = min(sum(dists.^2));

%frechMean = newMeshList{frechMean};

%% Shape Space Mean Approximation

meanVerts = zeros(size(newMeshList{1}.V));
for i = 1:length(newMeshList)
	meanVerts = meanVerts + newMeshList{i}.V;
end
meanVerts = meanVerts/length(newMeshList);
frechMean = Mesh('VF',meanVerts,newMeshList{1}.F);
...
disp([csvPath num2str(numGPLmks) '_GP_' num2str(numFPSLmks) '_FPS.csv']);
fid
